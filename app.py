...
# Teknisk relevante batteristørrelser: størrelser som når minimumskravet til
# teknisk udbytte uden at gå ud over det tekniske sweet spot.
minimum_benefit = technical.avoided_grid_import_kwh * (balance_capture / 100)
relevant_results = [
    r for r in results
...
if not relevant_results:
    relevant_results = [technical]
relevant_min = min(r.capacity_kwh for r in relevant_results)
relevant_max = max(r.capacity_kwh for r in relevant_results)
...
if len(relevant_results) > 1:
    st.markdown(
        f"**Relevant kapacitetsområde:** {relevant_min:g}–{relevant_max:g} kWh  ·  "
        f"**Anbefalet balance:** {combined.capacity_kwh:g} kWh"
    )
else:
    st.markdown(
        f"**Relevant kapacitetsvalg:** {relevant_min:g} kWh  ·  "
        f"**Anbefalet balance:** {combined.capacity_kwh:g} kWh"
    )
...
# Marker det teknisk relevante område diskret. Området består kun af
# faktiske produktstørrelser, som opnår minimumskravet til teknisk udbytte
# og ikke ligger over det tekniske sweet spot.
if relevant_max > relevant_min:
    fig.add_vrect(
        ...
        opacity=0.10,
        line_width=0,
        annotation_text=f"Relevant område: {relevant_min:g}–{relevant_max:g} kWh",
