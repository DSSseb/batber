...
with st.sidebar:
    ...
    balance_capture = st.slider(
        "Teknisk mål for balance",
        70,
        95,
        85,
        5,
        help="Målet er denne andel af netbesparelsen ved det tekniske sweet spot. En blød zone på 5 procentpoint under målet accepteres også, så økonomien kan afgøre, om de sidste få procent er merprisen værd.",
    )
...
c1.metric("Periode", f"{meta['start']:%d.%m.%Y} – {(meta['end'] - timedelta(hours=1)):%d.%m.%Y}")
...
# Anbefalingsområdet er bevidst smallere end hele den teknisk kvalificerede
# mængde. Det starter ved den anbefalede balance og kan højst gå ét reelt
# produkttrin op, hvis næste størrelse fortsat har en rimelig marginal
# udnyttelse og ikke ligger over det tekniske sweet spot.
#
# Den marginale grænse genbruger samme princip som det tekniske sweet spot:
# næste trin skal levere mindst den valgte andel af seriens bedste marginale
# udnyttelse. På den måde er markeringen rådgivningsorienteret uden at
# introducere endnu en uafhængig tærskel i brugerfladen.
positive_marginals = [
    r.marginal_per_added_kwh for r in results[1:]
    if r.marginal_per_added_kwh > 0
]
peak_marginal = max(positive_marginals) if positive_marginals else 0.0
range_cutoff = peak_marginal * (threshold / 100)

combined_index = next(
    (i for i, r in enumerate(results) if abs(r.capacity_kwh - combined.capacity_kwh) < 1e-9),
    0,
)
recommendation_range = [combined]
if combined_index + 1 < len(results):
    candidate = results[combined_index + 1]
    if (
        candidate.capacity_kwh <= technical.capacity_kwh + 1e-9
        and candidate.marginal_per_added_kwh + 1e-9 >= range_cutoff
    ):
        recommendation_range.append(candidate)

relevant_min = min(r.capacity_kwh for r in recommendation_range)
relevant_max = max(r.capacity_kwh for r in recommendation_range)
...
with col3:
    ...
    <div>Økonomisk stærkeste størrelse i eller tæt på den valgte tekniske målzone.</div></div>""",
        unsafe_allow_html=True,
    )
...
why_text = (
    ...
    f"som det tekniske sweet spot på {technical.capacity_kwh:g} kWh giver. Målet er {balance_capture} %, "
    f"men størrelser ned til {max(0, balance_capture - 5)} % indgår i en blød målzone, så økonomien kan afgøre, "
    f"om det er værd at betale for de sidste få procent teknisk udbytte."
)
...
st.caption(
    "Balance-reglen prioriterer kundens tekniske behov først, men bruger en blød målzone i stedet for en hård grænse. "
    "Med et mål på 85 % kan størrelser fra 80 % også vurderes, hvorefter økonomien vælger mellem de teknisk relevante alternativer. "
    "Lade-/afladeeffekt tilføjes senere, når effektdata er komplette."
)

if len(recommendation_range) > 1:
    st.markdown(
        f"**Anbefalingsområde:** {relevant_min:g}–{relevant_max:g} kWh  ·  "
        f"**Anbefalet balance:** {combined.capacity_kwh:g} kWh"
    )
else:
    st.markdown(
        f"**Anbefalet kapacitetsvalg:** {relevant_min:g} kWh  ·  "
        f"**Anbefalet balance:** {combined.capacity_kwh:g} kWh"
    )
...
# Kundevenlig PDF-rapport bruger præcis de samme resultater som dashboardet.
report_pdf = generate_customer_report(
    ...
    roundtrip_efficiency_pct=rte,
    min_soc_pct=min_soc,
    balance_capture_pct=balance_capture,
    range_min=relevant_min,
    range_max=relevant_max,
)
st.download_button(
    "📄 Download kunderapport (PDF)",
    data=report_pdf,
    file_name=f"batterianbefaling_{selected_brand}_{selected_model}_{combined.capacity_kwh:g}kWh.pdf".replace(" ", "_"),
    mime="application/pdf",
    use_container_width=False,
)
st.caption("Rapporten indeholder ingen målepunkts-ID eller andre identifikationsfelter fra Excel-filen.")
...
# Marker det snævre anbefalingsområde diskret. Det starter ved den
# anbefalede balance og omfatter højst næste realistiske produkttrin.
if relevant_max > relevant_min:
    fig.add_vrect(
        x0=relevant_min,
        x1=relevant_max,
