...
with col3:
    st.markdown(f"""<div class="reco"><div class="small">ANBEFALET BALANCE</div>
    <div style="font-size:2rem;font-weight:800">{combined.capacity_kwh:g} kWh</div>
    <div>Største trin hvor den ekstra kapacitet stadig er både teknisk og økonomisk rimeligt udnyttet.</div></div>""", unsafe_allow_html=True)


# Forklar hvorfor balancen stopper, hvis der findes et større trin.
next_result = None
...
if next_result is not None:
    st.info(
        f"Hvorfor stoppe ved {combined.capacity_kwh:g} kWh? Næste trin ({next_result.capacity_kwh:g} kWh) "
        f"giver {next_result.marginal_avoided_import_kwh:.0f} kWh ekstra reduceret netkøb for "
        f"{next_result.added_capacity_kwh:g} kWh ekstra batteri, svarende til "
        f"{next_result.marginal_per_added_kwh:.1f} kWh pr. ekstra batteri-kWh. "
        f"Merprisen er {next_result.marginal_price_dkk:,.0f} kr. og den ekstra økonomiske værdi "
        f"i måleperioden er {next_result.marginal_economic_value_dkk:,.0f} kr."
        .replace(",", ".")
    )
else:
    st.info("Den anbefalede balance er den største tilgængelige størrelse i den valgte batteriserie.")

st.caption(
    "Balance-reglen er stadig under kalibrering. Den vurderer hvert ekstra batteritrin på marginal teknisk udnyttelse "
    "og marginal økonomisk effektivitet i stedet for en 50/50 totalscore."
)
...
for r in results:
    ...
    if r.combined_recommended: labels.append("Balance")
