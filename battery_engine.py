...
def choose_combined_recommendation(
    ...
    *,
    min_marginal_technical_kwh_per_kwh: float = 35.0,
    min_economic_retention_fraction: float = 0.65,
) -> SimulationResult:
    """Choose the practical balance by evaluating each *next* battery step.

    The old prototype used a 50/50 score between total technical benefit and total
    economic efficiency. That tended to over-reward the cheapest battery. This rule
    instead starts at the economically strongest product and only moves upward while
    the next step still has acceptable marginal value.

    A larger battery is accepted only when:
      1) it is not larger than the technical sweet spot,
      2) the extra capacity avoids at least ``min_marginal_technical_kwh_per_kwh``
         kWh of grid import per added battery-kWh over the measurement period, and
      3) the marginal economic efficiency retains at least
         ``min_economic_retention_fraction`` of the best positive upgrade step.

    This makes the stopping point transparent and keeps unequal capacity steps fair.
    """
    if not results:
        raise ValueError("Ingen simuleringsresultater.")
    ...
    # Only consider products up to the technical sweet spot.
    ...
    # Start at the economic recommendation when it is within the technical envelope.
    start_index = 0
    for i, r in enumerate(eligible):
        if abs(r.capacity_kwh - economic.capacity_kwh) < 1e-9:
            start_index = i
            break

    positive_marginal_econ = [
        r.marginal_value_per_1000_dkk
        for r in eligible[1:]
        if r.marginal_price_dkk > 0 and r.marginal_value_per_1000_dkk > 0
    ]
    best_marginal_econ = max(positive_marginal_econ, default=0.0)
    econ_cutoff = best_marginal_econ * min_economic_retention_fraction

    recommendation = eligible[start_index]
    for i in range(start_index + 1, len(eligible)):
        candidate = eligible[i]
        tech_ok = candidate.marginal_per_added_kwh >= min_marginal_technical_kwh_per_kwh
        econ_ok = (
            candidate.marginal_price_dkk > 0
            and candidate.marginal_economic_value_dkk > 0
            and (best_marginal_econ <= 0 or candidate.marginal_value_per_1000_dkk >= econ_cutoff)
        )
        if tech_ok and econ_ok:
