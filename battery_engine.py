...
def choose_combined_recommendation(
    ...
    *,
    ...
    soft_zone_width_fraction: float = 0.05,
) -> SimulationResult:
    """Choose a practical balance using a *soft* technical target zone.

    ``min_technical_capture_fraction`` is the preferred technical target (normally
    85 % of the avoided grid import delivered by the technical sweet spot). Instead
    of treating 84.9 % as a failure and 85.0 % as a pass, the model opens a soft
    zone immediately below the target. With the default 5 percentage-point width,
    products from 80-85 % are still allowed to compete economically.

    The recommendation is therefore the economically strongest product inside the
    soft-or-better technical zone, while never exceeding the technical sweet spot.
    This makes neighbouring product series behave more consistently when their
    module steps land on opposite sides of an arbitrary threshold.
    """
    if not results:
        raise ValueError("Ingen simuleringsresultater.")
    ...
    preferred_fraction = max(0.0, min(1.0, min_technical_capture_fraction))
    soft_floor_fraction = max(0.0, preferred_fraction - max(0.0, soft_zone_width_fraction))
    soft_minimum_benefit = target_benefit * soft_floor_fraction

    # Technical need still comes first: only products close enough to the technical
    # sweet spot may compete. The soft zone simply avoids a binary cliff at 85 %.
    eligible = [
        r for r in results
        if r.capacity_kwh <= technical.capacity_kwh + 1e-9
        and r.avoided_grid_import_kwh + 1e-9 >= soft_minimum_benefit
    ]
