...
# Brug ReportLabs indbyggede fonte. De kræver ingen systemfontfiler og fungerer derfor
# både lokalt og på Streamlit Community Cloud.
...
def generate_customer_report(*, brand: str, model: str, results: Sequence,
    ...
    title = ParagraphStyle('TitleDSS', parent=styles['Title'], fontName=FONT_BOLD,
                           fontSize=23, leading=27, textColor=DARK, spaceAfter=4*mm)
    h1 = ParagraphStyle('H1DSS', parent=styles['Heading1'], fontName=FONT_BOLD,
                        fontSize=14, leading=18, textColor=DARK, spaceBefore=3*mm, spaceAfter=2*mm)
    body = ParagraphStyle('BodyDSS', parent=styles['BodyText'], fontName=FONT_REGULAR,
                          fontSize=9.5, leading=14, textColor=DARK)
    small = ParagraphStyle('SmallDSS', parent=body, fontSize=8, leading=11, textColor=MID)
    hero_label = ParagraphStyle('HeroLabel', parent=small, fontName=FONT_BOLD,
                                fontSize=8, textColor=MID, alignment=TA_LEFT)
    hero_value = ParagraphStyle('HeroValue', parent=body, fontName=FONT_BOLD,
                                fontSize=23, leading=26, textColor=BRAND)
    card_value = ParagraphStyle('CardValue', parent=body, fontName=FONT_BOLD,
    ...
    story = []
    story.append(Paragraph('DANSK SOLCELLESERVICE', ParagraphStyle('BrandHead', parent=small, fontName=FONT_BOLD, textColor=BRAND, fontSize=9)))
    ...
    for idx, r in enumerate(results, start=1):
        if abs(r.capacity_kwh-combined.capacity_kwh)<1e-9:
            comparison.setStyle(TableStyle([('BACKGROUND',(0,idx),(-1,idx),PALE_ORANGE), ('FONTNAME',(0,idx),(-1,idx),FONT_BOLD)]))
            break
    ...
    at.setStyle(TableStyle([
        ...
        ('FONTNAME',(0,0),(0,-1),FONT_BOLD), ('FONTNAME',(1,0),(1,-1),FONT_REGULAR),
